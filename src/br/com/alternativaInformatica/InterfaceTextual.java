...
public class InterfaceTextual {
	...
	private void cadastrarCliente() {
		String tmpNome, tmpContato, tmpEndereco, tmpFone, tmpBairro, tmpCep, tmpCidade, tmpEstado, tmpCnpj, tmpIe,
				tmpEmail;
		...
		try {
			GerenciadorAplicacao.getInstance().cadastrarCliente(new Cliente(tmpNome, tmpContato, tmpEndereco, tmpFone,
					tmpBairro, tmpCep, tmpCidade, tmpEstado, tmpCnpj, tmpIe, tmpEmail));
...
//			String tmpId = Integer.toString(GerenciadorAplicacao.getInstance().getCountServicos() + 1);
			...
			GerenciadorAplicacao.getInstance().cadastrarServico(new Servico(tmpDescricao, tmpPreco));
	...
	private void criarOrdemServico(boolean preenchida) throws ClassNotFoundException, SQLException {
		GerenciadorAplicacao tmpGerenciador = GerenciadorAplicacao.getInstance();
		boolean tmpSalvar = true;
		byte tmpOpcao=0;

		tmpGerenciador.limparOrdemServico();
		tmpGerenciador.setNumeroOs(idos);
		idos++;
		...
		if (tmpSalvar) {
			...
			} while (tmpOpcao != 1 && tmpOpcao != 2 && tmpOpcao != 3);
		}
	}

	private byte selecionarCliente() throws ClassNotFoundException, SQLException {
		GerenciadorAplicacao.getInstance().carregarClientes();
		System.out.println();
	...
	private byte selecionarServico() throws ClassNotFoundException, SQLException {
		GerenciadorAplicacao.getInstance().carregarServicos();
		System.out.println();
